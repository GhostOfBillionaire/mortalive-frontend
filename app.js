...
/* Show only on mobile */
@media (max-width: 640px) {
  body.di2-live.di2-authenticated:not(.di2-on-landing):not(.di2-on-auth) #di2-bot {
    display: flex !important;
  }
}
...
`;
  ...
  function syncBottomNavAuthVisibility() {
    ...
    if (nav) {
      ...
      // Do not write display/visibility/pointer-events inline here.
      // The CSS contract above owns visual visibility so a transient auth
      // hydration state cannot permanently hide the nav on app pages.
